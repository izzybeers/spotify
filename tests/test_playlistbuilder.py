...
def test_create_change_remove_reactivate_multibandlikedsongs_playlist():
    ...
    try:
        ...
        assert playlist_id in followed_playlists['id'].values, 'Error: user is not following playlist.'
        ...
        followed_playlists = get_followed_playlists(sp)
        assert playlist_id in followed_playlists['id'].values, 'Error: user is not following new playlist.'
        ...
        followed_playlists = get_followed_playlists(sp)
        assert playlist_id in followed_playlists['id'].values, 'Error: user is not following trimmed playlist.'
        ...
        followed_playlists = get_followed_playlists(sp)
        assert playlist_id not in followed_playlists['id'].values, 'Error: User is still following the deactivated playlist'
        ...
        followed_playlists = get_followed_playlists(sp)
        assert playlist_id in followed_playlists['id'].values, 'Error: user is not following reactivated playlist.'
...
def test_create_change_remove_reactivate_multibandtopstreams_playlist():
    user_id = sp.current_user()['id']
    user_name = sp.current_user()['name']
    band_list = ['All That Remains', 'Arch Enemy']
    print('Attempting to create playlist for All That Remains and Arch Enemy...')
    sub_id = add_new_playlist(playlist_type = 'TopStreamsMultiBands',
                                public = True,
                                user_id = user_id,
                                user_name = user_name,
                                band_list = band_list,
