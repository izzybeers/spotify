    ...
    if chunk_size is not None:
        ...
        while True:
            if eq_col_name is None:
                res = pd.DataFrame(supabase.table(table_name).select(select).range(start_indx, start_indx + chunk_size).execute().data)
            else:
                res = pd.DataFrame(supabase.table(table_name).select(select).eq(eq_col_name, eq_value).range(start_indx, start_indx + chunk_size).execute().data)
...
def get_followed_playlists():
    ...
    while True:
        ...
        res_to_add = [item['id'] for item in res['items']]
        followed_playlists = followed_playlists + res_to_add
...
def get_songs_on_playlist(playlist_id):
